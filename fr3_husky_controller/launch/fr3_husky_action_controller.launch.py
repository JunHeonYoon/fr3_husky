...
def _launch_setup(context, *args, **kwargs):
    ...
    if is_dual:
        _set_controller_params('dual_fr3_husky_action_controller')
    else:
        controller_name = 'left_fr3_husky_action_controller' if normalized_sides[0] == 'left' else 'right_fr3_husky_action_controller'
    ...
    for broadcaster_name in franka_broadcaster_names:
        if broadcaster_name not in cm_params:
            raise RuntimeError(
                f"Controller '{broadcaster_name}' not found in fr3_husky_ros_controllers.yaml controller_manager."
            )
    ...
    load_controllers = []
    if is_dual:
        controllers = ['dual_fr3_husky_action_controller', 'joint_state_broadcaster']
    else:
        controller_name = 'left_fr3_husky_action_controller' if normalized_sides[0] == 'left' else 'right_fr3_husky_action_controller'
